...
export const getAllCarriersController = async (req, res) => {
  try {
    ...
    getAllCarriers(async (err, results) => {
      if (err)
        return res.status(500).json({ message: "Error fetching carriers" });

      await redisClient.set("carriers:all", JSON.stringify(results), {
        EX: 300,
      }); // Caché por 5 minutos
  ...
  try {
    ...
    getCarrierById(carrierId, async (err, results) => {
      if (err)
        return res
          .status(500)
          .json({ message: "Error fetching carrier by ID" });
      if (results.length === 0)
...
export const createCarrierController = (req, res) => {
  ...
  const { name, vehicle_capacity, is_available, location, vehicle_type } =
    req.body;
  if (
    (!name || !vehicle_capacity || !is_available, !location || !vehicle_type)
  ) {
    return res.status(400).json({ message: "todos los campos son obligatorios para carries" });
  }
...
export const updateCarrierByIdController = async(req, res) => {
  ...
  const { name, vehicle_capacity, is_available, location, vehicle_type } = req.body;
  // if (!name || vehicle_capacity == null || is_available == null) {
  //   return res.status(400).json({ message: "All fields are required" });
  // }

  const updatedCarrier = { name, vehicle_capacity, is_available, location, vehicle_type  };
